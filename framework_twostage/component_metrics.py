...
"""
Warning: Didn't implement the IOU matching within the single frame-level
in our compute_component_maps() function.

TO_DO: Finish compute_component_maps()
"""
...
class TripletDisentangle:
    def __init__(self, map_path="/ssd/prostate/dataset_triplet/triplet_maps.txt"):
        if not os.path.exists(map_path):
            raise FileNotFoundError(f"Mapping file {map_path} does not exist")
        self.mapping = {}
    ...
    def extract(self, inputs, component="i"):
        ...
        for i, triplet_idx in enumerate(inputs):
            if not (0 <= triplet_idx < self.num_triplets):
                raise ValueError(f"无效的triplet类别索引: {triplet_idx}")
