...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data=None, task="detect", is_triplet=False, cls_type="triplet", **kwargs):
        """
        ...
        Args:
            ...
            is_triplet (bool): Whether to use triplet format labels.
            cls_type (str): Classification type, either 'triplet' or 'tool'.
        ...
        self.data = data
        self.is_triplet = is_triplet # triplet format label
        self.cls_type = cls_type # classification type (triplet_id or tool_id)
        ...
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(
                ...
                iterable=zip(
                    ...
                    repeat(self.is_triplet), 
                    repeat(self.data.get("nc_tool", 0)),         # 传递nc_tool参数
                    repeat(self.data.get("nc_action", 0)),       # 传递nc_action参数
                    repeat(self.data.get("nc_target", 0)),
                    repeat(self.data.get("nc_triplet", 0)),
                    repeat(self.cls_type),                       # 传递cls_type参数
                ),
            )
            ...
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                ...
                if not self.is_triplet:
                    if im_file: # 是否文件损坏，如果损坏则会是[None, None, None, None, None, nm, nf, ne, nc, msg]
                                # 如果损坏, 那么这个样本就会被跳过，不会添加到标签列表中 
                        x["labels"].append(
                            {
                                "im_file": im_file,
                                "shape": shape,
                        ...
                        x["labels"].append(
                            {
                                "im_file": im_file,
                                "shape": shape,
                                "cls": lb[:, 0:1] if self.cls_type == "triplet" else lb[:, 1:2],  # triplet_id or tool_id
                                "triplet_id": lb[:, 0:1],  # n, 1  总是保存triplet_id
                                "tool_id": lb[:, 1:2],  # n, 1
                                "action_id": lb[:, 2:3],  # n, 1
                                "target_id": lb[:, 3:4],  # n, 1
                                "bboxes": lb[:, 4:8],  # n, 4
                                "segments": segments,
                                "keypoints": keypoint,
                                "normalized": True,
    ...
    def build_transforms(self, hyp=None):
        ...
        transforms.append(
            Format(
                ...
                is_triplet=self.is_triplet,  # 传递is_triplet参数
            )
        )
        return transforms
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb", "tool_id", "action_id", "target_id", "triplet_id"}:
