...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4, is_triplet=False):
        """
        ...
        Args:
            ...
            p (float): Probability of applying the mosaic augmentation. Must be in the range 0-1.
            n (int): The grid size, either 4 (for 2x2) or 9 (for 3x3).
            is_triplet (bool): 是否使用triplet格式（具有tool_id, action_id, target_id字段）。
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            # 使用self.is_triplet判断是否使用triplet格式
            if self.is_triplet and "tool_id" in labels:
                tool_ids.append(labels["tool_id"])
                action_ids.append(labels["action_id"])
                target_ids.append(labels["target_id"])
                triplet_ids.append(labels["triplet_id"])
        # Final labels
        final_labels = {
        ...
        # 使用self.is_triplet判断是否是triplet格式
        if self.is_triplet and tool_ids:
            final_labels["tool_id"] = np.concatenate(tool_ids, 0)
            final_labels["action_id"] = np.concatenate(action_ids, 0)
            final_labels["target_id"] = np.concatenate(target_ids, 0)
            final_labels["triplet_id"] = np.concatenate(triplet_ids, 0)
        ...
        # 使用self.is_triplet判断是否是triplet格式
        if self.is_triplet and "tool_id" in final_labels:
            final_labels["tool_id"] = final_labels["tool_id"][good]
            final_labels["action_id"] = final_labels["action_id"][good]
            final_labels["target_id"] = final_labels["target_id"][good]
            final_labels["triplet_id"] = final_labels["triplet_id"][good]
...
class MixUp(BaseMixTransform):
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0, is_triplet=False) -> None:
        """
    ...
    def _mix_transform(self, labels):
        ...
        # 支持triplet格式
        if self.is_triplet:
            if "tool_id" in labels and "tool_id" in labels2:
                labels["tool_id"] = np.concatenate([labels["tool_id"], labels2["tool_id"]], 0)
                labels["action_id"] = np.concatenate([labels["action_id"], labels2["action_id"]], 0)
                labels["target_id"] = np.concatenate([labels["target_id"], labels2["target_id"]], 0)
                labels["triplet_id"] = np.concatenate([labels["triplet_id"], labels2["triplet_id"]], 0)
...
class RandomPerspective:
    ...
    def __init__(
        self, degrees=0.0, translate=0.1, scale=0.5, shear=0.0, perspective=0.0, border=(0, 0), pre_transform=None, is_triplet=False
    ):
        """
        ...
        Args:
            ...
            pre_transform (Callable | None): Function/transform to apply to the image before starting the random
                transformation.
            is_triplet (bool): Whether to use triplet format data (with tool_id, action_id, target_id fields).
    ...
    def __call__(self, labels):
        ...
        # 过滤triplet格式的标签，使用相同的索引i
        if self.is_triplet:
            labels["tool_id"] = tool_id[i]
            labels["action_id"] = action_id[i]
            labels["target_id"] = target_id[i]
            labels["triplet_id"] = triplet_id[i]
...
class Format:
    ...
    def __init__(
        ...
        batch_idx=True,
        bgr=0.0,
        is_triplet=False,  # 添加is_triplet参数
    ):
        """
        ...
        self.bgr = bgr
        self.is_triplet = is_triplet  # 保存is_triplet参数
    ...
    def __call__(self, labels):
        ...
        nl = len(instances)
        if self.is_triplet:
            tool_id = labels.pop("tool_id")
            action_id = labels.pop("action_id")
            target_id = labels.pop("target_id")
            triplet_id = labels.pop("triplet_id")
        ...
        # new labels and old labels
        if nl: # 如果存在实例
            labels["bboxes"] = torch.from_numpy(instances.bboxes)
            labels["cls"] = torch.from_numpy(cls)
            if self.is_triplet:
                labels["tool_id"] = torch.from_numpy(tool_id)
                labels["action_id"] = torch.from_numpy(action_id)
                labels["target_id"] = torch.from_numpy(target_id)
                labels["triplet_id"] = torch.from_numpy(triplet_id)
                
        else: # 背景图
            labels["bboxes"] = torch.zeros((0, 4))  
            labels["cls"] = torch.zeros((0))  # 为空标签添加空的cls张量
            if self.is_triplet:
                labels["tool_id"] = torch.zeros((0))
                labels["action_id"] = torch.zeros((0))
                labels["target_id"] = torch.zeros((0))
                labels["triplet_id"] = torch.zeros((0))
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    """
    mosaic = Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic, is_triplet=dataset.is_triplet)
    ...
    if hyp.copy_paste_mode == "flip":
        ...
        pre_transform.append(
            CopyPaste(
                dataset,
                pre_transform=Compose([Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic, is_triplet=dataset.is_triplet), affine]),
                p=hyp.copy_paste,
    ...
    return Compose(
        [
            pre_transform,
            MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup, is_triplet=dataset.is_triplet),
