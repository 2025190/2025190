...
# 处理txt中的所有行
# 第一次运行的时候，缓存不存在，所以会执行self.cache_labels()调用verify_image_label
# 第二次运行的时候，缓存存在，所以会执行self.get_labels()
# 所以记得删除缓存，在错误的时候
# Also need to change Format.__call__ in augment.py
def verify_image_label(args):
    """验证一个图像-标签对。"""
    # 解包参数 nkpt: number of keypoints; ndim: number of dimensions;
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, single_cls, is_triplet, nc_tool, nc_action, nc_target, nc_triplet, cls_type = args
    
    # 数值统计：缺失、找到、空、损坏、消息、段落、关键点
    nm, nf, ne, nc, msg, segments, keypoints = 0, 0, 0, 0, "", [], None
    
    try:
        # 验证图像
        im = Image.open(im_file)  # 尝试用PIL打开图像
        im.verify()  # 验证图像文件完整性
        shape = exif_size(im)  # 获取图像尺寸，考虑EXIF旋转信息
        shape = (shape[1], shape[0])  # 转换为(高度,宽度)格式
        
        # 确保图像尺寸合理
        assert (shape[0] > 9) & (shape[1] > 9), f"图像尺寸{shape} <10像素"
        
        # 验证图像格式
        assert im.format.lower() in IMG_FORMATS, f"无效的图像格式{im.format}"
        
        # 处理JPEG特殊情况，检查文件结束标记是否正确
        if im.format.lower() in {"jpg", "jpeg"}:
            with open(im_file, "rb") as f:
                f.seek(-2, 2)  # 移动到倒数第二个字节
                if f.read() != b"\xff\xd9":  # 如果没有JPEG结束标记
                    # 修复损坏的JPEG并保存
                    ImageOps.exif_transpose(Image.open(im_file)).save(im_file, "JPEG", subsampling=0, quality=100)
                    msg = f"{prefix}{im_file}: 损坏的JPEG已修复并保存"

        # 验证标签文件
        if os.path.isfile(lb_file):  # 如果标签文件存在
            nf = 1  # 标记已找到标签
            with open(lb_file, encoding="utf-8") as f:
                # 读取所有行并分割，排除空行 # 类别ID (tripletid): 存储在 lb[:, 0:1] 中
                # 边界框坐标 (yolobbox): 存储在 lb[:, 1:] 中
                lb = [x.split() for x in f.read().strip().splitlines() if len(x)]
                
                # # 检查是否包含分割标注（每行>6个值且不是关键点）
                # if any(len(x) > 6 for x in lb) and (not keypoint):
                #     # 提取类别ID
                #     classes = np.array([x[0] for x in lb], dtype=np.float32)
                #     # 提取分割点坐标，每行的第2个值开始为分割点
                #     segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in lb]
                #     # 将分割转换为边界框，连接类别ID与边界框
            ...
            if nl := len(lb):  # 如果有标签
                ...
                if keypoint:
                    ...
                    points = lb[:, 5:].reshape(-1, ndim)[:, :2]  # 提取关键点坐标
                elif is_triplet:
                    assert lb.shape[1] == 8, f"标签需要8列, 检测到{lb.shape[1]}列"
                    points = lb[:, 5:]  # 提取关键点坐标
                else:
                ...
                if is_triplet:
                    ...
                    if nc_triplet > 0:
                        max_cls = lb[:, 0].max()
                        assert max_cls < num_cls, f"标签triplet_id {int(max_cls)}超过数据集类别数{num_cls}"
                    
                    # 验证tool_id
                    if nc_tool > 0:
                        max_tool = lb[:, 1:2].max()
                        assert max_tool < nc_tool, f"标签tool_id {int(max_tool)}超过工具类别数{nc_tool}"
                    
                    # 验证action_id
                    if nc_action > 0:
                        max_action = lb[:, 2:3].max()
                        assert max_action < nc_action, f"标签action_id {int(max_action)}超过动作类别数{nc_action}"
                    
                    # 验证target_id
                    if nc_target > 0:
                        max_target = lb[:, 3:4].max()
                        assert max_target < nc_target, f"标签target_id {int(max_target)}超过目标类别数{nc_target}"
                else:
                    max_cls = lb[:, 0].max()  # 最大类别ID
                    assert max_cls < num_cls, f"标签类别{int(max_cls)}超过数据集类别数{num_cls}"
                ...
                if not is_triplet:
                    ...
                    if len(i) < nl:  # 如有重复
                        lb = lb[i]  # 只保留唯一标签
                    if segments:
                        segments = [segments[x] for x in i]  # 更新分割数据
                    msg = f"{prefix}{im_file}: 移除了{nl - len(i)}个重复标签"
                ...
                if keypoint:
                    ...
                    lb = np.zeros((0, 8), dtype=np.float32)  # triplet格式需要8列
                else:
                    lb = np.zeros((0, 5), dtype=np.float32)
        else:
            nm = 1  # 标记为缺失标签 # txt文件不存在
            if keypoint:
                lb = np.zeros((0, (5 + nkpt * ndim)), dtype=np.float32)
            elif is_triplet:
                lb = np.zeros((0, 8), dtype=np.float32)  # triplet格式需要8列
        ...
        # 处理关键点数据
        if keypoint:
            keypoints = lb[:, 5:].reshape(-1, nkpt, ndim)  # 重塑关键点数据
            if ndim == 2:
                # 创建可见性掩码：若x<0或y<0，设为不可见(0)，否则为可见(1)
                kpt_mask = np.where((keypoints[..., 0] < 0) | (keypoints[..., 1] < 0), 0.0, 1.0).astype(np.float32)
                keypoints = np.concatenate([keypoints, kpt_mask[..., None]], axis=-1)  # 添加可见性通道
        
        # 根据格式决定保留多少列
        if is_triplet:
            lb = lb[:, :8]  # 保留8列: triplet_id, tool_id, action_id, target_id, x, y, w, h
        else:
            lb = lb[:, :5]  # 标准格式只保留5列: class_id, x, y, w, h
        
        # 返回处理结果：图像文件、处理后的标签、图像尺寸、分割数据、关键点、统计信息和消息
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg
    
    except Exception as e:
        # 处理异常情况
        nc = 1  # 标记为损坏
        msg = f"{prefix}{im_file}: 忽略损坏的图像/标签: {e}"
        # 返回空值和错误消息
