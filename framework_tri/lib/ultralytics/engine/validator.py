...
from ultralytics.utils.ops import Profile, scale_boxes, xywh2xyxy
...
class BaseValidator:
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        ...
        self.roi_aligns = None
        self.layers_to_extract = None
        self.spatial_scales = None
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if yolotriplet_prepare:       
            ...
            model_to_hook = de_parallel(model)  
            self.feature_extractor.register_hooks(model_to_hook, self.layers_to_extract)
            
            roi_size = getattr(self.args, 'roi_size', 7)
            ...
            self.roi_aligns = {
                layer_name: RoIAlign(
                    ...
                    sampling_ratio=2,
                    aligned=True
                ) for layer_name, scale in self.spatial_scales.items()
            }
            print("已成功初始化特征提取器和ROI Align模块")

        is_triplet = self.args.is_triplet
        # one epoch
        for batch_i, batch in enumerate(bar):
            ...
            # Preprocess
            with dt[0]:
                batch = self.preprocess(batch, is_triplet)
            ...
            # Postprocess
            with dt[3]:
                preds = self.postprocess(preds) # non_max_suppression

            # Update metrics for every batch
            self.update_metrics(preds, batch) # Batch TP/FP/FN
            if is_triplet:
                self.update_metrics_triplet(preds, batch)
        ...
        stats = self.get_stats() # Cal AP/mAP
        if is_triplet:
            video_stats = self.get_triplet_stats() # TO_DO: save in self.metrics
            self.print_triplet_stats(video_stats)
        self.check_stats(stats) # Check Nan
        self.speed = dict(zip(self.speed.keys(), (x.t / len(self.dataloader.dataset) * 1e3 for x in dt)))
        
        # 清理特征提取器的钩子
        if self.feature_extractor is not None:
            self.feature_extractor.remove_hooks()
            LOGGER.info("removed feature extraction hooks")
            
        self.finalize_metrics()
        if self.training:
            self.print_results() # print each class
        ...
        if self.training:
            ...
            total_time = sum(self.speed.values())  
            fps = 1000 / total_time if total_time > 0 else 0  
            LOGGER.info(f"FPS: {fps:.1f} (frames/second)")
            
            stats["metrics/FPS"] = round(fps, 2)
    ...
    def update_metrics_triplet(self, preds, batch):
        """Update metrics based on predictions and batch for triplet evaluation."""
        pass
